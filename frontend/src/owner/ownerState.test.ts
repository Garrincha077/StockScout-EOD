...
import {clearOwnerLocalStorage,clearOwnerPasswordRecoveryLocation,LEGACY_OWNER_WATCHLIST_KEY,nextOwnerWatchlist,normalizeOwnerTicker,ownerPasswordRecoveryRedirect,parseOwnerJsonObject,PASSWORD_RESET_REQUEST_NOTICE,watchlistAfterSessionChange} from './ownerState.ts'
...
test('password recovery redirects to the app root from GitHub Pages ticker links',()=>{
  assert.equal(
    ownerPasswordRecoveryRedirect({origin:'https://garrincha077.github.io',pathname:'/StockScout-EOD/ticker/NVDA'}),
    'https://garrincha077.github.io/StockScout-EOD/?owner-recovery=1',
  )
  assert.equal(
    ownerPasswordRecoveryRedirect({origin:'http://127.0.0.1:4173',pathname:'/'}),
    'http://127.0.0.1:4173/?owner-recovery=1',
  )
})

test('recovery marker cleanup preserves canonical run and hash',()=>{
  assert.equal(
    clearOwnerPasswordRecoveryLocation('https://example.test/StockScout-EOD/?run=scan-1&owner-recovery=1#owner'),
    '/StockScout-EOD/?run=scan-1#owner',
  )
  assert.match(PASSWORD_RESET_REQUEST_NOTICE,/If that email can reset StockScout access/i)
  assert.doesNotMatch(PASSWORD_RESET_REQUEST_NOTICE,/exists|not found|unknown user/i)
})
