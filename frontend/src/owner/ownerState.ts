...
export const PASSWORD_RESET_REQUEST_NOTICE='If that email can reset StockScout access, a recovery link has been sent.'
...
export function ownerPasswordRecoveryRedirect(location:{origin:string;pathname:string}){
  const url=new URL(ownerAppRoot(location.pathname),location.origin)
  url.searchParams.set('owner-recovery','1')
  return url.toString()
}

export function clearOwnerPasswordRecoveryLocation(href:string){
  const url=new URL(href)
  url.searchParams.delete('owner-recovery')
