...
import {builtInScreens,calculatePositionSize,fieldDefs,invalidRules,levelFitsExpandedCandleBounds,makeGroup,makeRule,matchesGroups,normalizeEodDate,opsByKind,preserveScanOrder,stockScoutFocusBlend,validateRule,weekStartUtc,type Logic,type RuleGroup,type ScreenState} from './deepvue/filterEngine'
...
import {isManifestV1,type ScanHistoryItemV1,type TradePlanV1,type TradeStatus} from './data/contracts'
...
type Page='Screener'|'Grid'|'Changes'|'Excluded'|'History'|'Watchlist'|'Market'|'Owner'
type Interval='D'|'W'
...
type RowsLoadState<T>={status:'idle'|'loading'|'ready'|'error';rows:T[];error?:string}
export type Stock={
  ...
  id?:string;scanOrder?:number;excluded?:boolean;focusBlend?:number
  tradeStatus?:TradeStatus;entryRiskPct?:number|null;tacticalStopLevel?:number|null
...
const defaultVisibility:VisibilityState={opportunityScore:false,opportunityRank:false,exclusionReasons:false,originalTTPasses:false,originalVcpQuality:false,originalAdVolumeRatio:false,originalRiskPct:false,originalSellScore:false,rsFromHigh:false,volumeDryUp:false,baseWeeks:false,distance30w:false,structureScore:false,baseScore:false,triggerScore:false,neglectedScore:false,avgDollarVolume20:false,fundamentalSupport:false,fundamentalEvidenceCoverage:false,opportunityGroupModifier:false,opportunityFundModifier:false,opportunityPenalty:false,emergingLeaderScore:false,revenueYoY:false,epsYoY:false,operatingCashFlowYoY:false,freeCashFlowYoY:false,freeCashFlowMargin:false,totalDebtYoY:false,netDebt:false,shareDilutionYoY:false,leadershipScore:false,groupRank:false,groupRS:false,groupConfidence:false}
...
const stockScout=(s:Stock)=>stockScoutFocusBlend(s as Record<string,unknown>)
const tradePlanOf=(s:Stock)=>{
  const plan=s.tradePlan||{}
  const status=(s.tradeStatus??plan.status??plan.trade_status??'insufficient_data') as TradeStatus
  const rawTactical=s.tacticalStopLevel??plan.tacticalStopLevel??plan.tactical_stop_level
  const numeric=(value:unknown)=>value!==null&&value!==undefined&&Number.isFinite(Number(value))?Number(value):null
  return{
    status,
    trigger:numeric(plan.triggerReferenceLevel??plan.trigger_reference_level),
    entry:numeric(plan.entryReferenceLevel??plan.entry_reference_level),
    structural:numeric(plan.structuralInvalidationLevel??plan.structural_invalidation_level),
...
const tradeLabel:Record<TradeStatus,string>={entry_ready:'Entry ready',trigger_pending:'Trigger pending',wait_for_retest:'Wait for retest',not_tradeable:'Not tradeable',insufficient_data:'Insufficient data'}
function TradeBadge({stock}:{stock:Stock}){const status=tradePlanOf(stock).status;return<span className={`trade-badge ${status}`}>{tradeLabel[status]}</span>}
const externalChartUrl=(ticker:string)=>(import.meta.env.VITE_EXTERNAL_CHART_URL||'https://www.tradingview.com/symbols/{ticker}/').replace('{ticker}',encodeURIComponent(ticker))
...
function aggregateWeekly(bars:Bar[]){const out:Bar[]=[];for(const b of bars){const key=weekStartUtc(b.time);if(!key)continue;const last=out[out.length-1];if(!last||last.time!==key)out.push({...b,time:key});else{last.high=Math.max(last.high,b.high);last.low=Math.min(last.low,b.low);last.close=b.close;last.volume+=b.volume;last.rs=b.rs}}return out}
...
function chartSource(bars:Bar[],interval:Interval,range:Range){return(interval==='W'?aggregateWeekly(bars):bars).slice(-rangeCount(range,interval))}
function chartBar(row:any):Bar|null{
  ...
  if(!time||![open,high,low,close,volume,rs].every(Number.isFinite))return null
  return{time,open,high,low,close,volume,rs}
}

function PriceChart({bars,interval='W',range='5Y',mode='Price',mini=false,stock}:{bars:Bar[];interval?:Interval;range?:Range;mode?:ChartMode;mini?:boolean;stock?:Stock}){
  ...
  useEffect(()=>{
    ...
    if(mode==='Price'){
      ...
      if(!mini&&stock){
        ...
        if(plan.trigger!=null)c.createPriceLine({price:plan.trigger,color:'#62a8ff',lineWidth:2,lineStyle:LineStyle.Dashed,axisLabelVisible:true,title:'Entry trigger'})
        if(plan.tactical!=null)c.createPriceLine({price:plan.tactical,color:'#ff6f7d',lineWidth:2,lineStyle:LineStyle.Solid,axisLabelVisible:true,title:'Tactical stop'})
        if(plan.structural!=null&&levelFitsExpandedCandleBounds(plan.structural,source,.1)&&plan.structural!==plan.tactical)c.createPriceLine({price:plan.structural,color:'#d99a62',lineWidth:1,lineStyle:LineStyle.Dotted,axisLabelVisible:true,title:'Structural invalidation'})
      }
      ...
      for(const[n,color,kind]of specs as[number,string,'ema'|'sma'][]){const vals=kind==='ema'?ema(closes,n):ma(closes,n),line=chart.addSeries(LineSeries,{color,lineWidth:mini?1:2,priceLineVisible:false,lastValueVisible:false});line.setData(source.map((b,i)=>vals[i]==null?null:{time:b.time,value:vals[i]}).filter(Boolean) as any)}
...
function DeepVueTerminal(){
  const{core,error,manifest,selectedTicker,selectTicker,reviewScope,reload,loadChart,loadCandidateDetail,loadExcluded,loadHistory}=useStockScoutData()
  ...
  const[sorting,setSorting]=useState<SortingState>(()=>loadLocal('dv-sorts-v2',[]))
  ...
  const[excludedState,setExcludedState]=useState<RowsLoadState<Stock>>({status:'idle',rows:[]})
  const[historyState,setHistoryState]=useState<RowsLoadState<ScanHistoryItemV1>>({status:'idle',rows:[]})
  ...
  useEffect(()=>setVisibility(current=>({...current,exclusionReasons:page==='Excluded'})),[page])
  useEffect(()=>localStorage.setItem('dv-sorts-v2',JSON.stringify(sorting)),[sorting])
  ...
  const loadBars=useCallback(async(ticker:string,retry=false):Promise<ChartLoadState>=>{
    let result=await loadChart(ticker,retry)
    if(result.status!=='ready'&&owner.user&&manifest&&isManifestV1(manifest)){
      try{
        const rows=await owner.loadPrivateChart(manifest.runId,ticker)
        if(rows?.length)result={status:'ready',rows}
      }catch(nextError){return{status:'error',bars:[],error:nextError instanceof Error?nextError.message:String(nextError)}}
    }
    if(result.status!=='ready')return{...result,bars:[]}
    const bars=result.rows.map(chartBar).filter((bar):bar is Bar=>bar!==null).sort((left,right)=>left.time.localeCompare(right.time))
    return bars.length?{status:'ready',bars}:{status:'error',bars:[],error:'Chart payload has no valid EOD bars'}
  },[loadChart,owner.user,owner.loadPrivateChart,manifest])

  const snapshotKey=manifest&&isManifestV1(manifest)?`${manifest.runId}:${manifest.generatedAt}`:payload?.generatedAt??''
  useEffect(()=>{setExcludedState({status:'idle',rows:[]});setHistoryState({status:'idle',rows:[]})},[snapshotKey])
  useEffect(()=>{
    if(page!=='Excluded')return
    let live=true;setExcludedState({status:'loading',rows:[]})
    loadExcluded().then(rows=>{
      if(!live)return
      const normalized=rows.map((row:any,index)=>({
        ...
        excluded:true,
        reasons:row.reasons??row.reasonCodes??row.reason_codes??row.exclusionReasons??row.exclusion_reasons??[],
      } as Stock)).filter(row=>row.ticker)
      setExcludedState({status:'ready',rows:preserveScanOrder(normalized)})
    }).catch(next=>{if(live)setExcludedState({status:'error',rows:[],error:next instanceof Error?next.message:String(next)})})
    return()=>{live=false}
  },[page,loadExcluded,snapshotKey])
  useEffect(()=>{
    if(page!=='History')return
    let live=true;setHistoryState({status:'loading',rows:[]})
    loadHistory().then(rows=>{
      if(!live)return
      const latest=[...rows].sort((left,right)=>String((right as any).sessionDate??(right as any).session_date??'').localeCompare(String((left as any).sessionDate??(left as any).session_date??''))).slice(0,252)
      setHistoryState({status:'ready',rows:latest})
    }).catch(next=>{if(live)setHistoryState({status:'error',rows:[],error:next instanceof Error?next.message:String(next)})})
    return()=>{live=false}
  },[page,loadHistory,snapshotKey])
  useEffect(()=>{if(page==='Owner'&&!owner.user)setPage('Screener')},[page,owner.user])

  const universe=payload?.universe||[]
  const displayUniverse=page==='Excluded'?excludedState.rows:universe
  const watchlist=owner.watchlist
  const toggleWatch=(ticker:string)=>{void owner.toggleWatch(ticker).catch(()=>undefined)}
  const filtered=useMemo(()=>displayUniverse.filter(s=>{
  ...
  }),[displayUniverse,reviewScope,page,watchlist,query,recipe,groups,rootLogic])
  const sortedData=useMemo(()=>sorting.length?applyMultiSort(filtered,sorting):preserveScanOrder(filtered),[filtered,sorting])
  const candidatePage=!['Market','History','Owner'].includes(page)
  const selected=candidatePage?(sortedData.find(s=>s.ticker===selectedTicker)||sortedData[0]):undefined
  useEffect(()=>{
    if(!selected||selected.excluded||!manifest||!isManifestV1(manifest)){setSelectedDetail(null);return}
    let live=true;setSelectedDetail(null)
    loadCandidateDetail(selected.ticker).then(detail=>{if(live&&detail)setSelectedDetail(detail as Stock)}).catch(()=>undefined)
    return()=>{live=false}
  },[selected?.ticker,loadCandidateDetail,manifest])
  const selectedFull=selected&&selectedDetail?.ticker===selected.ticker?{...selected,...selectedDetail}:selected
  useEffect(()=>{if(candidatePage&&sortedData.length&&selectedTicker!==selected?.ticker)selectTicker(sortedData[0].ticker)},[candidatePage,sortedData,selectedTicker,selected?.ticker,selectTicker])
  ...
  const columns=useMemo(()=>[
    helper.display({id:'watch',header:'',enableSorting:false,cell:({row})=><button className={`dv-star ${watchlist.includes(row.original.ticker)?'on':''}`} disabled={!owner.user||row.original.excluded} title={!owner.user?'Owner sign-in required':row.original.excluded?'Excluded rows cannot be watched':'Toggle owner watchlist'} onClick={e=>{e.stopPropagation();toggleWatch(row.original.ticker)}}>★</button>}),
    helper.accessor('scanOrder',{header:'Scan #',cell:i=>Number.isFinite(Number(i.getValue()))?`#${Number(i.getValue())+1}`:'—'}),
    helper.accessor('ticker',{header:'Ticker',cell:i=><b className="dv-ticker">{i.getValue()}</b>}),
    helper.accessor(s=>stockScout(s),{id:'focusBlend',header:'StockScout',cell:i=><b className="dv-score">{fmt(i.getValue(),1)}</b>}),
    helper.accessor(s=>tradePlanOf(s).status,{id:'tradeStatus',header:'Trade status',cell:i=><TradeBadge stock={i.row.original}/>}),
    helper.accessor(s=>tradePlanOf(s).risk,{id:'entryRiskPct',header:'Entry risk',cell:i=>i.getValue()==null?'—':`${fmt(i.getValue(),1)}%`}),
    helper.accessor(s=>opp(s),{id:'opportunityScore',header:'Opportunity analysis',cell:i=><b>{fmt(i.getValue(),0)}</b>}),
    ...
    helper.accessor('opportunityRank',{header:'Opp %ile',cell:i=><b className={num(i.getValue())>=95?'dv-good':''}>{fmt(i.getValue(),0)}</b>}),
    ...
    helper.display({id:'exclusionReasons',header:'Excluded because',enableSorting:false,cell:({row})=><div className="dv-changechips">{(row.original.reasons||[]).slice(0,3).map(reason=><span key={reason}>{String(reason).replaceAll('_',' ')}</span>)}</div>}),
  ...
  const exportCsv=()=>{const cols=table.getAllLeafColumns().filter(c=>!['watch','todaySignals'].includes(c.id)&&c.getIsVisible());const lines=[cols.map(c=>c.id).join(','),...sortedData.map(s=>cols.map(c=>JSON.stringify((s as any)[c.id]??(c.id==='focusBlend'?stockScout(s):c.id==='opportunityScore'?opp(s):c.id==='primarySetup'?setupOf(s):''))).join(','))];const u=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv'})),a=document.createElement('a');a.href=u;a.download=`stockscout-${(page==='Excluded'?'excluded':reviewScope?`review-${reviewScope}`:activeScreen).replace(/\W+/g,'-').toLowerCase()}-${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(u)}

  if(!payload)return <div className="dv-loading">{error||'Loading StockScout…'}</div>
  const m=payload.market||{},daily=m.dailyChanges||{},ageH=Math.max(0,Math.round((Date.now()-new Date(payload.generatedAt).getTime())/3600000)),regimeLabel=marketRegimeLabel(m)
  const scanDate=manifest&&isManifestV1(manifest)?manifest.sessionDate:payload.generatedAt.slice(0,10)
  const scanStatus=manifest&&isManifestV1(manifest)?manifest.status:'legacy'
  const navPages:Page[]=['Screener','Grid','Changes','Excluded','History','Watchlist','Market',...(owner.user?['Owner' as const]:[])]
  return <div className="dv-app">
    <header className="dv-top"><div className="dv-brand">◉ <b>STOCKSCOUT EOD</b><small>{scanDate} · {scanStatus}</small></div><nav>{navPages.map(p=><button key={p} className={page===p?'active':''} onClick={()=>setPage(p)}>{p}{p==='Changes'&&daily.changed?` ${daily.changed}`:p==='Excluded'&&manifest&&isManifestV1(manifest)?` ${manifest.counts.excluded}`:p==='History'&&historyState.rows.length?` ${historyState.rows.length}`:''}</button>)}</nav><div className={`dv-live ${ageH>36?'stale':''}`}><b>{regimeLabel}</b><span>{universe.length.toLocaleString()} stocks</span><span>{ageH}h old</span><button onClick={reload} aria-label="Refresh scan">↻</button></div></header>

    {page==='Market'?<Market universe={universe} market={m}/>:page==='History'?<HistoryView state={historyState}/>:page==='Owner'?<Suspense fallback={<div className="dv-loading">Loading owner workspace…</div>}><OwnerWorkspace ticker={selectedTicker??''}/></Suspense>:<>
      <section className="dv-screenbar">{page==='Excluded'?<><div className="dv-screenpick"><span>EXCLUDED</span><b>Sanitized rejected records</b></div><div className="dv-screenmeta"><b>{excludedState.status==='loading'?'Loading…':`${excludedState.rows.length.toLocaleString()} records`}</b><span>risk and exclusion labels remain visible</span><span>{sorting.length?`${sorting.length} explicit sort levels`:'StockScout scan order'}</span></div></>:<><div className="dv-screenpick"><span>SCREEN</span><select value={allScreens.some(s=>s.name===activeScreen)?activeScreen:''} onChange={e=>{const s=allScreens.find(x=>x.name===e.target.value);if(s)applyScreen(s)}}><option value="">Custom</option>{builtInScreens.map(s=><option key={s.name}>{s.name}</option>)}{customScreens.length>0&&<optgroup label="My screens">{customScreens.map(s=><option key={s.name}>{s.name}</option>)}</optgroup>}</select><button onClick={saveScreen} disabled={invalidRuleCount>0} title={invalidRuleCount?'Fix invalid rules before saving':''}>Save as…</button>{customScreens.some(s=>s.name===activeScreen)&&<button className="danger" onClick={deleteScreen}>Delete</button>}</div><div className="dv-screenmeta"><b>{reviewScope?`Review · ${reviewScope==='today'?'Today':'New since last scan'}`:activeScreen}</b><span className={!reviewScope&&invalidRuleCount?'dv-rule-warning':''}>{reviewScope?'screen membership paused':invalidRuleCount?`${invalidRuleCount} invalid · ignored`:`${groups.reduce((n,g)=>n+g.rules.length,0)} rules`}</span><span>{sorting.length?`${sorting.length} explicit sort levels`:'StockScout scan order'}</span><span>{filtered.length.toLocaleString()} matches</span></div></>}</section>

      {page!=='Excluded'&&<section className="dv-recipes">{recipeTabs.map(t=><button key={t} className={recipe===t?'active':''} onClick={()=>setRecipe(t)}>{t}<small>{t==='All'?universe.length:universe.filter(s=>tagsOf(s).includes(t)).length}</small></button>)}</section>}

      <section className="dv-sortbar"><span>{sorting.length?'EXPLICIT SORT':'STOCKSCOUT'}</span>{sorting.length?sorting.map((s,i)=><div className="dv-sortchip" key={s.id}><b>{i+1}</b><em>{s.id===EMA_FRESH_SORT?'EMA 10/20 · Fresh':s.id===SMA_FRESH_SORT?'SMA 10/20 · Fresh':String(table.getColumn(s.id)?.columnDef.header||s.id)}</em><button title={isFreshMaSort(s.id)?'Fresh crossover first':s.desc?'Strength descending':'Strength ascending'} onClick={()=>cycleSort(s.id)}>{isFreshMaSort(s.id)?'✨':s.desc?'↓':'↑'}</button><button disabled={i===0} onClick={()=>moveSort(s.id,-1)}>‹</button><button disabled={i===sorting.length-1} onClick={()=>moveSort(s.id,1)}>›</button><button onClick={()=>setSorting(x=>x.filter(y=>y.id!==s.id))}>×</button></div>):<i>Canonical scan order · click a column only when you want an explicit sort</i>}<button className="dv-clear" onClick={()=>setSorting([])}>Scan order</button></section>
      ...
      {builderOpen&&page!=='Excluded'&&<FilterBuilder rootLogic={rootLogic} setRootLogic={setRootLogic} groups={groups} setGroups={setGroups} addGroup={addGroup} updateGroup={updateGroup} removeGroup={removeGroup}/>}
      {columnsOpen&&<ColumnPicker table={table} setVisibility={setVisibility}/>}

      {excludedState.status==='error'&&page==='Excluded'?<div className="dv-loading">Excluded records could not be loaded: {excludedState.error}</div>:excludedState.status==='loading'&&page==='Excluded'?<div className="dv-loading">Loading excluded records…</div>:page==='Grid'?<GridView stocks={sortedData} count={gridCount} setCount={setGridCount} range={gridRange} setRange={setGridRange} loadBars={loadBars} selected={selected?.ticker} onSelect={selectTicker} watchlist={watchlist} toggleWatch={toggleWatch} canWatch={Boolean(owner.user)}/>:<main className="dv-work"><div className="dv-tablebox"><div className="dv-tablewrap"><table><thead>{table.getHeaderGroups().map(hg=><tr key={hg.id}>{hg.headers.map(h=>{const fid=maFreshSortId(h.column.id),si=sorting.findIndex(s=>s.id===h.column.id||(fid!==null&&s.id===fid)),ss=si>=0?sorting[si]:null;return <th key={h.id} className={si>=0?'sorted':''} onClick={()=>h.column.getCanSort()&&cycleSort(h.column.id)}>{flexRender(h.column.columnDef.header,h.getContext())}{si>=0&&<><i>{si+1}</i><b>{ss&&isFreshMaSort(ss.id)?'✨':ss?.desc?'↓':'↑'}</b></>}</th>})}</tr>)}</thead><tbody>{table.getRowModel().rows.map(r=><tr key={r.id} className={r.original.ticker===selected?.ticker?'selected':''} onClick={()=>selectTicker(r.original.ticker)}>{r.getVisibleCells().map(c=><td key={c.id}>{flexRender(c.column.columnDef.cell,c.getContext())}</td>)}</tr>)}</tbody></table></div><footer><span>{page==='Changes'?'Meaningful changes since the previous scan':page==='Excluded'?'Excluded records · no silent hiding':sorting.length?'Explicit user-selected sort':'Canonical StockScout scan order'}</span><div><button disabled={!table.getCanPreviousPage()} onClick={()=>table.previousPage()}>←</button><b>{pagination.pageIndex+1}/{Math.max(1,table.getPageCount())}</b><button disabled={!table.getCanNextPage()} onClick={()=>table.nextPage()}>→</button></div></footer></div>{selectedFull&&<Detail stock={selectedFull} chart={selectedChart} retryChart={()=>loadSelectedChart(true)} interval={interval} setInterval={setInterval} range={range} setRange={setRange} mode={chartMode} setMode={setChartMode} watched={watchlist.includes(selectedFull.ticker)} canWatch={Boolean(owner.user)&&!selectedFull.excluded} toggleWatch={()=>toggleWatch(selectedFull.ticker)}/>}</main>}
    </>}
  </div>
}
...
function ColumnPicker({table,setVisibility}:{table:any;setVisibility:(v:VisibilityState)=>void}){
  const sets:Record<string,VisibilityState>={Core:defaultVisibility,Opportunity:{...defaultVisibility,opportunityScore:true,opportunityTier:true,opportunityRank:true,opportunityPotential:true,opportunityTiming:true,opportunityGroupModifier:true,opportunityFundModifier:true,opportunityPenalty:true,emergingLeaderScore:true},Early:{...defaultVisibility,prior9mReturn:true,stage2AgeWeeks:true,neglectedScore:true,atrCompression:false,vcpScore:false},Crosses:{...defaultVisibility,ema10d:true,ema20d:true,ema10d20dCrossAge:true,ema10d20dSpreadPct:true,sma10w:true,sma20w:true,sma10w20wCrossAge:true,sma10w20wSpreadPct:true},Groups:{...defaultVisibility,leadershipScore:true,groupRank:true,groupRS:true,groupConfidence:true},Breakout:{...defaultVisibility,breakoutPct:true,volumeRatio:true,triggerScore:true,atrCompression:true},Base:{...defaultVisibility,vcpScore:true,atrCompression:true,tightRange20:true,baseWeeks:true,baseScore:true},Fundamentals:{...defaultVisibility,fundamentalEvidenceScore:true,fundamentalEvidenceConfidence:true,fundamentalEvidenceCoverage:true,fundamentalSupport:true,revenueYoY:true,epsYoY:true,operatingCashFlowYoY:true,freeCashFlowYoY:true,freeCashFlowMargin:true,totalDebtYoY:true,netDebt:true,shareDilutionYoY:true},Changes:{...defaultVisibility,changeImpact:true,opportunityDelta:true,rsRankDelta:true,todaySignals:true}}
...
function GridView({stocks,count,setCount,range,setRange,loadBars,selected,onSelect,watchlist,toggleWatch,canWatch}:{stocks:Stock[];count:number;setCount:(n:number)=>void;range:Range;setRange:(r:Range)=>void;loadBars:(t:string,retry?:boolean)=>Promise<ChartLoadState>;selected?:string;onSelect:(t:string)=>void;watchlist:string[];toggleWatch:(ticker:string)=>void;canWatch:boolean}){
  ...
  return <main className="dv-gridview"><header><div><b>RAPID REVIEW</b><span>{visible} of {stocks.length.toLocaleString()} current matches · explicit user sort or canonical StockScout scan order</span></div><div><select value={selectValue} onChange={e=>setCount(Number(e.target.value))}>{showProgress&&<option value={count}>{count}</option>}{presets.map(n=><option key={n} value={n}>{n}</option>)}{!presets.includes(stocks.length)&&<option value={stocks.length}>All ({stocks.length})</option>}</select>{(['6M','1Y','2Y','5Y'] as Range[]).map(r=><button className={range===r?'active':''} key={r} onClick={()=>setRange(r)}>{r}</button>)}</div></header><section className="dv-chartgrid">{stocks.slice(0,visible).map(s=><MiniCard key={s.ticker} stock={s} range={range} loadBars={loadBars} selected={selected===s.ticker} watched={watchlist.includes(s.ticker)} canWatch={canWatch&&!s.excluded} toggleWatch={()=>toggleWatch(s.ticker)} onClick={()=>onSelect(s.ticker)}/>)}</section><div className="dv-grid-sentinel" ref={sentinel} aria-hidden="true"/></main>
}
function MiniCard({stock,range,loadBars,selected,watched,canWatch,toggleWatch,onClick}:{stock:Stock;range:Range;loadBars:(t:string,retry?:boolean)=>Promise<ChartLoadState>;selected:boolean;watched:boolean;canWatch:boolean;toggleWatch:()=>void;onClick:()=>void}){
  ...
  return <article className={`dv-minicard ${selected?'selected':''}`} onClick={onClick}><header><div><b>{stock.ticker}</b><span>{setupOf(stock)}</span><TradeBadge stock={stock}/></div><div className="dv-miniactions"><button className={`dv-heart ${watched?'on':''}`} disabled={!canWatch} aria-label={canWatch?(watched?`Remove ${stock.ticker} from watchlist`:`Add ${stock.ticker} to watchlist`):'Owner sign-in required for watchlist'} title={canWatch?(watched?'Remove from watchlist':'Add to watchlist'):'Owner sign-in required'} onClick={e=>{e.stopPropagation();if(canWatch)toggleWatch()}}>{watched?'♥':'♡'}</button><strong title="StockScout focus blend">{fmt(stockScout(stock),1)}</strong></div></header><div className="dv-miniinfo"><span>Scan <b>#{num(stock.scanOrder)+1}</b></span><span>RS <b>{fmt(stock.rsRank,0)}</b></span><span>Fund <b>{fmt(stock.fundamentalEvidenceScore,0)}</b></span><span>Vol <b>{fmt(stock.volumeRatio,1)}x</b></span><span>10W <b>{signed(stock.distance10w)}</b></span>{stock.changeImpact!==undefined&&num(stock.changeImpact)!==0&&<span className={num(stock.changeImpact)>0?'dv-good':'dv-bad'}>Δ <b>{signed(stock.changeImpact,0)}</b></span>}</div>{state.status==='ready'?<PriceChart bars={state.bars} interval={interval} range={range} mini/>:state.status==='loading'?<div className="dv-miniload">loading chart…</div>:state.status==='unavailable'?<div className="dv-miniload"><span>Owner chart</span><a href={externalChartUrl(stock.ticker)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>Open external chart ↗</a></div>:<div className="dv-miniload"><button onClick={e=>{e.stopPropagation();setAttempt(x=>x+1)}}>retry chart</button></div>}<footer>{(stock.changeLabels||[]).slice(0,2).map(x=><span key={x}>{x}</span>)}</footer></article>
}

function PositionSizer({stock}:{stock:Stock}){
  const plan=tradePlanOf(stock)
  const[nav,setNav]=useState('10000000'),[riskPct,setRiskPct]=useState('0.5')
  const[entry,setEntry]=useState(()=>String(plan.entry??stock.price??''))
  useEffect(()=>setEntry(String(plan.entry??stock.price??'')),[stock.ticker,plan.entry,stock.price])
  const sizing=calculatePositionSize(plan.status,nav,riskPct,entry,plan.tactical)
  const money=(value:number)=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(value)
  const sectionStyle={margin:'0 0 8px',padding:9,border:'1px solid #1b354c',borderRadius:8,background:'#071722'}
  if(plan.status!=='entry_ready'||plan.tactical==null)return <section className="position-sizer" style={sectionStyle}><header><div><b>POSITION SIZER</b><small> · NAV default $10M · enabled only with entry-ready and a valid tactical stop.</small></div></header><div className="trade-levels"><span className="disabled"><small>Sizing unavailable</small><b>{sizing.reason}</b></span></div></section>
  const inputStyle={width:'100%',boxSizing:'border-box' as const,marginTop:3,padding:'5px 6px',border:'1px solid #27425e',borderRadius:5,background:'#07111d',color:'#eef7ff'}
  return <section className="position-sizer" style={sectionStyle}><header><div><b>POSITION SIZER</b><small> · Uses tactical stop only · structural invalidation is never a sizing fallback.</small></div></header><div className="trade-levels"><span><small>NAV (USD)</small><input aria-label="Portfolio NAV" type="number" min="1" step="1000" value={nav} style={inputStyle} onChange={event=>setNav(event.target.value)}/></span><span><small>Risk per trade (%)</small><input aria-label="Risk per trade percent" type="number" min="0.01" step="0.05" value={riskPct} style={inputStyle} onChange={event=>setRiskPct(event.target.value)}/></span><span><small>Entry price</small><input aria-label="Entry price" type="number" min="0.01" step="0.01" value={entry} style={inputStyle} onChange={event=>setEntry(event.target.value)}/></span><span><small>Tactical stop</small><b>${fmt(plan.tactical,2)}</b></span>{sizing.enabled?<><span><small>Shares</small><b>{sizing.shares.toLocaleString()}</b></span><span><small>Position</small><b>{money(sizing.positionValue)} · {fmt(sizing.positionPctNav,1)}% NAV</b></span><span><small>Planned risk</small><b>{money(sizing.riskUsed)} / {money(sizing.riskBudget)}</b></span><span><small>Risk / share</small><b>${fmt(sizing.riskPerShare,2)}{sizing.cashLimited?' · NAV capped':''}</b></span></>:<span className="disabled"><small>Sizing unavailable</small><b>{sizing.reason}</b></span>}</div></section>
}

function Detail({stock,chart,retryChart,interval,setInterval,range,setRange,mode,setMode,watched,canWatch,toggleWatch}:{stock:Stock;chart:ChartLoadState;retryChart:()=>void;interval:Interval;setInterval:(v:Interval)=>void;range:Range;setRange:(v:Range)=>void;mode:ChartMode;setMode:(v:ChartMode)=>void;watched:boolean;canWatch:boolean;toggleWatch:()=>void}){
  const plan=tradePlanOf(stock)
  ...
  const structuralOutside=plan.structural!=null&&chart.status==='ready'&&!levelFitsExpandedCandleBounds(plan.structural,chartSource(chart.bars,interval,range),.1)
  return <aside className="dv-detail"><div className="dv-detailhead"><button className={`dv-star big ${watched?'on':''}`} disabled={!canWatch} title={canWatch?'Toggle owner watchlist':'Owner sign-in required'} onClick={()=>{if(canWatch)toggleWatch()}}>★</button><div><h1>{stock.ticker}</h1><span>Stage {stock.stage} · {stock.stageName}</span><TradeBadge stock={stock}/></div><div className="dv-opp"><small>STOCKSCOUT · FOCUS BLEND</small><b>{fmt(stockScout(stock),1)}</b><span>Scan #{num(stock.scanOrder)+1}</span>{num(stock.opportunityDelta)!==0&&<span className={num(stock.opportunityDelta)>0?'dv-good':'dv-bad'}>Opp Δ {signed(stock.opportunityDelta,0)}</span>}</div><div className="dv-price"><b>${fmt(stock.price,2)}</b><span>{signed(stock.change20d)} 20D</span></div></div>
    ...
    <section className="trade-plan"><header><div><b>TRADE PLAN</b><small>Structural invalidation is not a position stop unless entry is ready.</small></div><TradeBadge stock={stock}/></header><div className="trade-levels"><span><small>Ulazni trigger</small><b>{plan.trigger==null?'—':`$${fmt(plan.trigger,2)}`}</b></span><span><small>Ulazna referenca</small><b>{plan.entry==null?'—':`$${fmt(plan.entry,2)}`}</b></span><span><small>Strukturna invalidacija</small><b>{plan.structural==null?'—':`$${fmt(plan.structural,2)}${structuralOutside?' · izvan grafa':''}`}</b></span><span className={plan.tactical==null?'disabled':''}><small>Tactical stop</small><b>{plan.tactical==null?'Nije definiran — sizing onemogućen':`$${fmt(plan.tactical,2)}`}</b></span><span><small>Entry risk</small><b>{plan.risk==null?'—':`${fmt(plan.risk,1)}%`}</b></span><span><small>Extension</small><b>{plan.extension==null?'—':`${fmt(plan.extension,2)} ATR`}</b></span></div>{plan.reasons.length>0&&<footer>{plan.reasons.map(reason=><span key={reason}>{String(reason).replaceAll('_',' ')}</span>)}</footer>}</section>
    ...
    <div className="dv-chartbox">{chart.status==='loading'?<div className="dv-chartmsg">Loading 5Y history…</div>:chart.status==='ready'?<PriceChart bars={chart.bars} interval={interval} range={range} mode={mode} stock={stock}/>:chart.status==='unavailable'?<div className="dv-chartmsg">Chart unavailable for this ticker in public mode. <a href={externalChartUrl(stock.ticker)} target="_blank" rel="noreferrer">Open external chart ↗</a></div>:<div className="dv-chartmsg">Chart request failed. <button onClick={retryChart}>Retry</button></div>}</div>
    <div className="dv-kpis"><K l="RS Rank" v={fmt(stock.rsRank,0)} d={stock.rsRankDelta}/><K l="RS Δ" v={fmt(stock.rsAcceleration,2)}/><K l="Group" v={fmt(stock.groupRank,0)}/><K l="Group RS" v={signed(stock.groupRS)}/><K l="Group conf" v={`${fmt(stock.groupConfidence,0)}%`}/><K l="TT" v={`${fmt(stock.trendTemplatePasses,0)}/8`}/><K l="EMA 10D" v={fmt(stock.ema10d,2)}/><K l="EMA 20D" v={fmt(stock.ema20d,2)}/><K l="D EMA X" v={stock.ema10d20dCross?`${stock.ema10d20dCross} ${fmt(stock.ema10d20dCrossAge,0)}d`:'—'}/><K l="D EMA spread" v={signed(stock.ema10d20dSpreadPct,2)}/><K l="SMA 10W" v={fmt(stock.sma10w,2)}/><K l="SMA 20W" v={fmt(stock.sma20w,2)}/><K l="W SMA X" v={stock.sma10w20wCross?`${stock.sma10w20wCross} ${fmt(stock.sma10w20wCrossAge,0)}w`:'—'}/><K l="W SMA spread" v={signed(stock.sma10w20wSpreadPct,2)}/><K l="S2 age" v={`${fmt(stock.stage2AgeWeeks,1)}w`}/><K l="Vol" v={`${fmt(stock.volumeRatio,2)}x`} d={stock.volumeRatioDelta}/><K l="Breakout" v={signed(stock.breakoutPct)}/><K l="10W" v={signed(stock.distance10w)}/><K l="30W" v={signed(stock.distance30w)}/><K l="Potential" v={fmt(stock.opportunityPotential,0)}/><K l="Timing" v={fmt(stock.opportunityTiming,0)}/><K l="Opp %ile" v={fmt(stock.opportunityRank,0)}/><K l="Group Δ" v={`${num(stock.opportunityGroupModifier)>0?'+':''}${fmt(stock.opportunityGroupModifier,1)}`}/><K l="Fund Δ" v={`${num(stock.opportunityFundModifier)>0?'+':''}${fmt(stock.opportunityFundModifier,1)}`}/><K l="Fund Ev" v={fmt(stock.fundamentalEvidenceScore,0)}/><K l="Fund conf" v={`${fmt(stock.fundamentalEvidenceConfidence,0)}%`}/></div>
...
function HistoryView({state}:{state:RowsLoadState<ScanHistoryItemV1>}){
  if(state.status==='loading'||state.status==='idle')return <div className="dv-loading">Loading up to 252 market sessions…</div>
  if(state.status==='error')return <div className="dv-loading">History could not be loaded: {state.error}</div>
  if(!state.rows.length)return <div className="dv-loading">No compact scan history is available for this snapshot.</div>
  const healthy=state.rows.filter(row=>(row as any).status==='healthy').length
  return <main className="dv-market"><section><h2>Scan history</h2><div className="dv-marketgrid"><div><b>{state.rows.length}</b><span>Sessions retained</span></div><div><b>{healthy}</b><span>Healthy</span></div><div><b>{state.rows.length-healthy}</b><span>Degraded / failed</span></div><div><b>252</b><span>Maximum sessions</span></div></div><p>Compact EOD history only. Prices are not live.</p></section><section><h2>Recent sessions</h2>{state.rows.slice(0,12).map((row:any)=><div className="dv-leader" key={row.runId??row.run_id}><b>{row.sessionDate??row.session_date}</b><span>{row.status}</span><em>{row.candidateCount??row.candidate_count??row.counts?.candidates??0} candidates</em><strong>{fmt(row.coveragePct??row.coverage_pct,1)}%</strong></div>)}</section><section><h2>All retained sessions</h2>{state.rows.map((row:any)=><div className="dv-leader" key={`all-${row.runId??row.run_id}`}><b>{row.sessionDate??row.session_date}</b><span>{row.status}</span><em>{row.excludedCount??row.excluded_count??row.counts?.excluded??0} excluded</em><strong>{fmt(row.coveragePct??row.coverage_pct,1)}%</strong></div>)}</section></main>
}

function Market({universe,market}:{universe:Stock[];market:Record<string,any>}){const daily=market.dailyChanges||{},stages=[1,2,3,4].map(s=>[s,universe.filter(x=>x.stage===s).length] as const),leaders=preserveScanOrder(universe).slice(0,20),changes=[...universe].filter(x=>x.changedToday).sort((a,b)=>num(b.changeImpact)-num(a.changeImpact)).slice(0,20);return <main className="dv-market"><section><h2>Market structure</h2><div className="dv-marketgrid">{stages.map(([s,n])=><div key={s}><b>{n}</b><span>Stage {s}</span></div>)}</div><p>Regime <b>{marketRegimeLabel(market)}</b> · Stage 2 breadth <b>{market.stage2Pct??0}%</b></p></section><section><h2>What changed since last scan</h2><div className="dv-marketgrid"><div><b>{daily.changed??0}</b><span>Changed</span></div><div><b>{daily.newSetups??0}</b><span>New setups</span></div><div><b>{daily.stageChanges??0}</b><span>Stage changes</span></div><div><b>{daily.rsMovers??0}</b><span>RS movers</span></div></div>{changes.slice(0,8).map(s=><div className="dv-leader" key={s.ticker}><b>{s.ticker}</b><span>{(s.changeLabels||[])[0]||'Changed'}</span><strong className={num(s.changeImpact)>0?'dv-good':'dv-bad'}>{signed(s.changeImpact,0)}</strong></div>)}</section><section><h2>StockScout scan order</h2>{leaders.slice(0,12).map(s=><div className="dv-leader" key={s.ticker}><b>{s.ticker}</b><span>{setupOf(s)}</span><em>Scan #{num(s.scanOrder)+1}</em><strong>{fmt(stockScout(s),1)}</strong></div>)}</section></main>}
