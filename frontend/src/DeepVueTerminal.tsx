...
function Market({universe,market}:{universe:Stock[];market:Record<string,any>}){const daily=market.dailyChanges||{},stages=[1,2,3,4].map(s=>[s,universe.filter(x=>x.stage===s).length] as const),leaders=preserveScanOrder(universe).slice(0,20),changes=[...universe].filter(x=>x.changedToday).sort((a,b)=>num(b.changeImpact)-num(a.changeImpact)).slice(0,20);return <main className="dv-market"><section><h2>Market structure</h2><div className="dv-marketgrid">{stages.map(([s,n])=><div key={s}><b>{n}</b><span>Stage {s}</span></div>)}</div><p>Regime <b>{market.regime||'Unknown'}</b> · Stage 2 breadth <b>{market.stage2Pct??0}%</b></p></section><section><h2>What changed since last scan</h2><div className="dv-marketgrid"><div><b>{daily.changed??0}</b><span>Changed</span></div><div><b>{daily.newSetups??0}</b><span>New setups</span></div><div><b>{daily.stageChanges??0}</b><span>Stage changes</span></div><div><b>{daily.rsMovers??0}</b><span>RS movers</span></div></div>{changes.slice(0,8).map(s=><div className="dv-leader" key={s.ticker}><b>{s.ticker}</b><span>{(s.changeLabels||[])[0]||'Changed'}</span><strong className={num(s.changeImpact)>0?'dv-good':'dv-bad'}>{signed(s.changeImpact,0)}</strong></div>)}</section><section><h2>StockScout scan order</h2>{leaders.slice(0,12).map(s=><div className="dv-leader" key={s.ticker}><b>{s.ticker}</b><span>{setupOf(s)}</span><em>Scan #{num(s.scanOrder)+1}</em><strong>{fmt(stockScout(s),1)}</strong></div>)}</section></main>}
