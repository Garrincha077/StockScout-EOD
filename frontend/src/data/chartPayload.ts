...
export function validateChartManifest(value:unknown,runId:string):ChartManifest{
  ...
  if(url.protocol!=='https:'||url.username||url.password||url.search||url.hash||!url.pathname.endsWith(`/storage/v1/object/public/stockscout-eod-charts/${runId}`))throw new Error('Chart storage URL is invalid')
