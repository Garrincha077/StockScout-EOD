...
DETAIL_BUCKETS = 128
HISTORY_LIMIT = 252
DEFAULT_PUBLIC_BASE_URL = "https://garrincha077.github.io/StockScout-EOD"
...
def _summary(
    ...
    *,
    ...
    return CandidateSummaryV1(
        id=_candidate_id(scan.run_id, ticker),
        canonicalUrl=_canonical_url(scan.run_id, ticker, public_base_url),
        ticker=ticker,
...
def _candidate_id(run_id: str, ticker: str) -> str:
    return f"scan:{run_id}:candidate:{ticker}"


def _canonical_url(run_id: str, ticker: str, public_base_url: str) -> str:
    base = public_base_url.strip().rstrip("/")
    parsed = urlsplit(base)
    if parsed.scheme != "https" or not parsed.netloc:
        raise ValueError("public_base_url must be an absolute HTTPS URL")
    return f"{base}/ticker/{quote(ticker, safe='.-')}?run={quote(run_id, safe='._-')}"
...
def _history_payload(
    ...
    return {
        ...
        "generatedAt": scan.generated_at,
        "sessions": sessions[:HISTORY_LIMIT],
    }
...
def build_public_snapshot(
    ...
    *,
    ...
    try:
        ...
        history = _history_payload(scan, health.status, previous_manifest)
