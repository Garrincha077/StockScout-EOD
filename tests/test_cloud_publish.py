...
def test_edge_and_sql_source_still_use_the_same_three_hash_contracts() -> None:
    root = Path(__file__).resolve().parents[1]
    edge = (
        root / "supabase" / "functions" / "stockscout-eod-publish" / "index.ts"
    ).read_text(encoding="utf-8")
    sql = next((root / "supabase" / "migrations").glob("*_cloud_security.sql")).read_text(
        encoding="utf-8"
    )
    assert "publishRecordHash(row)" in edge
    assert "key !== \"manifestHash\"" in edge
    assert "string_agg(record_hash" in sql
    assert "order by source, ticker" in sql


def test_python_wrapper_hash_matches_the_shared_edge_jcs_fixture() -> None:
    root = Path(__file__).resolve().parents[1]
    fixture = json.loads(
        (
            root
            / "supabase"
            / "functions"
            / "stockscout-eod-publish"
            / "jcs_fixture.json"
        ).read_text(encoding="utf-8")
    )
    assert stable_json_text(fixture["wrapper"]) == fixture["canonical"]
    assert publish_record_hash(fixture["wrapper"]) == fixture["sha256"]
